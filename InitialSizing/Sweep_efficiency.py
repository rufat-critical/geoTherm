...
def make_ORC(fluid, Pin, Tcool, Thot, w, PR_turb, eta_turb, eta_pump):
    ORC = gt.Model([gt.Boundary(name='LowT', fluid=fluid, P=Pin, T=Tcool),
                    gt.Rotor('ORC_Rotor', N =14009.97841),
                  gt.fixedFlowPump(name='Pump', rotor= 'ORC_Rotor', psi=0.45, eta=eta_pump, PR=6, w=w, US='LowT', DS='PumpOut'),
                  ...
                  gt.Turbine(name='Turb', rotor = 'ORC_Rotor', Ns= 7.4, Ds=2.4, eta=eta_turb, PR=PR_turb, w=w, US='TurbIn', DS='TurbOut'),
...
#fluids = ['isobutane','benzene','ammonia', 'cyclopentane','acetone','toluene','n-Hexane']
#fluids = ['isobutane', 'ammonia', 'propyne', 'butane','cis-2-Butene','Isohexane', 'propyne']
fluids = ['isobutane', 'propane','Propylene', 'butane','CycloHexane','CycloPropane']
...
for fluid in fluids:
    ...
    ORC, HOT, CombinedModel = make_ORC(fluid, Pin, Tcool, T_Turb, w, 2, eta_turb=0.75, eta_pump=0.7)
    ...
    for PR in PR_sweep:
        ...
        data['PR'].append(PR)
        data['PRPump'].append(ORC['Pump'].PR)
        data['PHi'].append(ORC['Pump'].PR*Pin/1e5)
...
for fluid in fluids:
    ...
    ORC, HOT, CombinedModel = make_ORC(fluid, Pin, Tcool, T_Turb, w, 2, eta_turb=0.82, eta_pump=0.8)
    ...
    fluid_data2[fluid] = data

color = ['b','g','r','c','m','y','k']

for i, fluid in enumerate(fluids):
    plt.plot(fluid_data[fluid]['PR'], fluid_data[fluid]['performance'][:,2],f'{color[i]}',label=fluid)
    plt.plot(fluid_data2[fluid]['PR'], fluid_data2[fluid]['performance'][:,2],f'{color[i]}--',label=fluid)
    
plt.xlabel('Turbine PR')
plt.ylabel(r'$\eta$')
plt.legend()

plt.figure()
for fluid in fluids:
    plt.plot(fluid_data[fluid]['PR'], fluid_data[fluid]['PRPump'],label=fluid)
plt.xlabel('Turbine PR')
plt.ylabel(r'Pump PR')
plt.legend()


np.save('optimal_efficiency',fluid_data)
