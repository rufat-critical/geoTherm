...
for fluid in fluids:
    ...
    ORC, HOT, CombinedModel = make_ORC(fluid, thermo._Pvap*1.1, Tcool, T_Turb, w, 2)


    data = {'PR': [], 'performance': [], 'WTurb': [], 'WPump': [], 'WPumpWater':[], 'netperformance': [],
            'pumpOutletT':[], 'waterOutletT':[], 'Q':[], 'Hwater':[], 'TurbOutletT':[]}


    PR_sweep = np.arange(2,min([Max_P/thermo._Pvap,60]),1)
