...
from .external.cylinder import ExternalCylinder
from geoTherm.common import inputParser, addQuantityProperty, units, logger
...
class Cylinder(Geometry):
    ...
    Do : float, optional
        Outer diameter [m]. If not specified, computed as Di + 2*t.
    t : float, optional
        Wall thickness [m]. If Do is provided, t is recalculated.
    dz : float, optional
    ...
    L  : Length [m]
    t  : Wall thickness [m]
    area : Cross-sectional flow area (based on Di) [m²]
    perimeter : Wetted perimeter (based on Di) [m]
    inner._surface : Inner wall lateral surface [m²]
    outer._surface : Outer wall lateral surface [m²]
    """

    _units = {'L': 'LENGTH', 'th': 'LENGTH'}

    @inputParser
    def __init__(self, Di: 'LENGTH',
                 L: 'LENGTH',
                 Do: 'LENGTH' = None,
                 th: 'LENGTH' = 0,
                 dz: 'LENGTH' = 0,
                 roughness: 'LENGTH' = 1e-4):
        ...
        self.inner = InternalCylinder(D=Di, L=L, roughness=roughness, dz=dz)
        self.outer = ExternalCylinder(Do, L, roughness, dz)
    ...
    def _state(self) -> dict:
        return {
            'Di': self.inner._state['D'],
            'L': self._L,
            'th': self._th,
            'dz': self._dz,
            'roughness': self.inner._state['roughness']
        }
